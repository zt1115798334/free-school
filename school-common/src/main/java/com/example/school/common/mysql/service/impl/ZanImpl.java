...
@AllArgsConstructor
@Service
public class ZanImpl implements Zan {
    ...
    public Map<Long, Long> zanState(Long userId, List<Long> topicId, Short topicType, Short zanType) {
        List<com.example.school.common.mysql.entity.Zan> zanList = zanRepository.findByUserIdAndTopicIdInAndTopicTypeAndZanTypeAndZanState(userId, topicId, topicType, zanType, ON);
        return zanList.stream().collect(groupingBy(com.example.school.common.mysql.entity.Zan::getTopicId, counting()));
    }

    @Override
    public Map<Long, Long> countZan(List<Long> topicId, Short topicType, Short zanType) {
        List<com.example.school.common.mysql.entity.Zan> zanList = zanRepository.findByTopicIdInAndTopicTypeAndZanTypeAndZanState(topicId, topicType, zanType, ON);
        return zanList.stream().collect(groupingBy(com.example.school.common.mysql.entity.Zan::getTopicId, counting()));
    }
    ...
    public Map<Long, List<RoUser>> zanUser(List<Long> topicId, Short topicType, Short zanType) {
        List<com.example.school.common.mysql.entity.Zan> zanList = zanRepository.findByTopicIdInAndTopicTypeAndZanTypeAndZanState(topicId, topicType, zanType, ON);
        List<Long> userIdList = zanList.stream().map(com.example.school.common.mysql.entity.Zan::getUserId).distinct().collect(toList());
        Map<Long, RoUser> userMap = userService.findMapRoUserByUserId(userIdList);
        return zanList.stream()
                .collect(groupingBy(com.example.school.common.mysql.entity.Zan::getTopicId,
    ...
    public void enableOnZan(Long topicId, Short topicType, Short zanType, Long toUserId, Long fromUserId) {
        Optional<com.example.school.common.mysql.entity.Zan> zanOptional = zanRepository.findByUserIdAndTopicIdAndTopicTypeAndZanType(toUserId, topicId, topicType, zanType);
        com.example.school.common.mysql.entity.Zan zan = zanOptional.orElse(new com.example.school.common.mysql.entity.Zan(toUserId, topicId, topicType, zanType, ON));
        zan.setZanState(ON);
    ...
    public void enableOffZan(Long topicId, Short topicType, Short zanType, Long toUserId, Long fromUserId) {
        Optional<com.example.school.common.mysql.entity.Zan> zanOptional = zanRepository.findByUserIdAndTopicIdAndTopicTypeAndZanType(toUserId, topicId, topicType, zanType);
        com.example.school.common.mysql.entity.Zan zan = zanOptional.orElse(new com.example.school.common.mysql.entity.Zan(toUserId, topicId, topicType, zanType, OFF));
        zan.setZanState(OFF);
        zanRepository.save(zan);
    }
}
